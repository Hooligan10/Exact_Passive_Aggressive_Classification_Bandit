...
class EPABF1:
	...
	def get_update_matrix(self, feature_vectors, calculated_label, predicted_label, true_label, probabilities,support_set1,l_tilde,labeltil,step_lambda1,c=0.01):
		update_matrix = self.init_weights()
...
class EPABF2:
	...
	def get_update_matrix(self, feature_vectors, calculated_label, predicted_label, true_label, probabilities,support_set2,l_tilde,labeltil,c=0.01):
		update_matrix = self.init_weights()
...
def main():
	...
	for k in range(0, iterations):
		print("Epoch: %d" %(int(k)))
		indices=np.random.choice(m,100000,replace=True)
		
		banditron = Banditron(d)
		epabf = EPABF(d)
		epabf1 = EPABF1(d)
		epabf2 = EPABF2(d)
		bpa = BPA(d)
		...
		for t in range(0,100000):
			feature_vectors, true_label = (features[indices[t],:]).tolist(),labels[indices[t]]
			banditron.runb(feature_vectors, true_label)
			...
			bpa.runph(feature_vectors, true_label)
