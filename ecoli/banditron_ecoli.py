...
import datetime
from gen_ecoli import data

SYNSEP_CATEGORY_MAPPING = ['cp', 'im', 'imL', 'imS', 'imU', 'om', 'omL', 'pp']
...
def main():
	...
	for k in range(0, iterations):
		...
		for t in range(0,100000):
			ran = random.randint(0,335)
			feature_vectors, true_labell = data['featurelist'][ran], data['truelabel'][ran]
			true_label = get_category_index(true_labell)
			...
			if ((t+1)%100) == 0:
				...
				roundsph.append(banditronph.number_of_rounds)
				...
				# error_listc.append(confidit.error_rate)

					# error_listmp.append(mp.error_rate)
	...
	# a = {'error_rate_epabf2': er2, 'rounds': rr2}
