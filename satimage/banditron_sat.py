...
import datetime
from gen_satimage import data

SYNSEP_CATEGORY_MAPPING = ['0', '1', '2', '3','4','5','6']
...
class EPABF1:
	...
	def __init__(self, gamma = 0.07, c = 0.1):
		self.gamma = gamma
		self.c = c
		self.dict_length = 36
		self.weights = self.init_weights()
	...
	def get_update_matrix(self, feature_vectors, calculated_label, predicted_label, true_label, probabilities,support_set1,l_tilde,labeltil,step_lambda1):
		update_matrix = self.init_weights()
		...
		if predicted_label in support_set1:
			for r in range(0,len(support_set1)):
				...
				step_lambda1[support_set1[r]] = min(self.c,temp1)
	...
	def run(self, feature_vectors, true_label):
		...
		if(lt_tilde[0] != 0.0):
			...
			for i in range(0,len(self.weights)):
				...
				if min(self.c,temp1) > 0:
					step_lambda1[labeltil[i]] = min(self.c,temp1)
				else:
			...
			while(j!= mods):
				...
				if min(self.c,temp1) > 0:
					step_lambda1[labeltil[j]] = min(self.c,temp1)
				else:
...
class EPABF2:
	...
	def __init__(self, gamma = 0.04, c = 0.1):
		self.gamma = gamma
		self.c = c
		self.dict_length = 36
		self.weights = self.init_weights()
	...
	def get_update_matrix(self, feature_vectors, calculated_label, predicted_label, true_label, probabilities,support_set2,l_tilde,labeltil):
		update_matrix = self.init_weights()
		...
		if predicted_label in support_set2:
			...
			den = ((a*a*len(support_set2)) + 1 - a + (1/(modxsq*2*self.c)))
			for r in range(0,len(support_set2)):
				step_lambda2[support_set2[r]] = ((l_tilde[support_set2[r]] + (num/den))/(modxsq*(1+(1/(modxsq*2*self.c)))))
		else:
			num = - (((a*a)*total_support_class_loss2))
			den = ((a*a*len(support_set2)) + 1 + (1/(modxsq*2*self.c)))
			for r in range(0,len(support_set2)):
				step_lambda2[support_set2[r]] = ((l_tilde[support_set2[r]] + (num/den))/(modxsq*(1+(1/(modxsq*2*self.c)))))
	...
	def run(self, feature_vectors, true_label):
		...
		### Determining Support Class set for EPABF-II
		mods = len(self.weights)
		if(lt_tilde[0] != 0.0):
			...
			while(j != mods):
				if predicted_label in support_set2:
					if a == 0.0:
						...
						temp1 = (l_tilde[predicted_label]/a) + ((((a*a*j)+1-a+(1/(modx*2*self.c)))/(a*a))*l_tilde[labeltil[j]])
						if(cm_loss2 < temp1):
							support_set2.append(labeltil[j])
							cm_loss2 += l_tilde[labeltil[j]]
							j += 1
					...
					if a == 0.0:
						...
						temp1 = ((((a*a*j)+1-a+(1/(modx*2*self.c)))/(a*a))*l_tilde[labeltil[j]])
						if(cm_loss2 < temp1):
							support_set2.append(labeltil[j])
							cm_loss2 += l_tilde[labeltil[j]]
							j += 1
...
class PA2:
	...
	def run(self, feature_vectors, true_label,c=0.1):
		...
		### Determining Support Classes for PA-II
		mods = len(self.weights)
...
def main():
	...
	ernew1 = [0.0 for x in range(0,1000)]
	...
	truel = []
	iterations = 10
	for varc in range(0,len(c_value)):
		for varp in range(0,len(param_value)):
			for k in range(0, iterations):
				print("LoopNo: %d & ParamNo: %d & Epoch: %d" %(int(varc), int(varp), int(k)))
				...
				for t in range(0,100000):
					if varp == 0:
						ran = random.randint(0,6434)
						feature_vectors1, true_label1 = data['featurelist'][ran], data['truelabel'][ran]
						feature_vectors = feature_vectors1
						true_label = true_label1

						feat.append(feature_vectors1)
						truel.append(true_label1)
					...
					epabf.run(feature_vectors, true_label-1)
					epabf1.run(feature_vectors, true_label-1)
					epabf2.run(feature_vectors, true_label-1)					
					pa.run(feature_vectors, true_label-1)
					pa1.run(feature_vectors, true_label-1)
					pa2.run(feature_vectors, true_label-1)
					bpa.run(feature_vectors, true_label-1)
