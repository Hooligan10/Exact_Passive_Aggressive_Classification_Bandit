...
import datetime
from gen_iris import data

SYNSEP_CATEGORY_MAPPING = ['0', '1', '2']
...
class EPABF1:
	...
	def __init__(self, gamma=0.023, c=0.1):
		self.gamma = gamma
		self.c = c
		self.dict_length = 4
		self.weights = self.init_weights()
		...
		for i in range(0,len(update_matrix)):
			...
			### w(y_tilde) = w_t(y_tilde) - sum(lambdas)*(x_t)*(1[y_tilde = y_t]/prob(y_tilde)-1)
				...
				for j in range(0,len(feature_vectors)):
					...
					# update_matrix[i][j] = feature_vectors[j] * sum_step_lambda1 * (a-1)
	...
	def run(self, feature_vectors, true_label):
		...
		### Determining Support Classes for EPABF-I
		support_set1 = []
...
class EPABF2:
	...
	def __init__(self, gamma = 0.035, c = 0.1):
		self.gamma = gamma
		self.c= c
		self.dict_length = 4
		self.weights = self.init_weights()
...
class BPA:
	...
	def run(self, feature_vectors, true_label):
		...
		update_matrix = self.get_update_matrixph(feature_vectors, predicted_label,total, losst, a)
		self.update_weights(update_matrix)
	...
	for varc in range(0,len(c_value)):
		for varp in range(0,len(param_value)):
			for k in range(0, iterations):
				print("Loop : %d LambdaNo: %d Epoch: %d" %(int (varc), int(varp),int(k)))

				banditron = Banditron()
				epabf = EPABF()
				epabf1 = EPABF1(2**(-5), 2**(-7.7))
				epabf2 = EPABF2()
