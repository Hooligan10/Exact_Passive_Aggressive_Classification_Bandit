...
import datetime
from gen_abalone import data


SYNSEP_CATEGORY_MAPPING = ['0', '1', '2', '3']

class Banditron:

	def __init__(self, gamma = 0.014):
		self.gamma = gamma
		self.dict_length = 7
...
class EPABF1:
	def __init__(self, gamma = 0.02, c = 0.1):
	...
	def run(self, feature_vectors, true_label):
		...
		### Determining Support Class for EPABF-I
		support_set1 = []
...
class EPABF2:
	...
	def __init__(self, gamma = 0.03, c = 0.1):
		# self.gamma = 0.03
		self.gamma = gamma
		self.c = c
		self.dict_length = 7
...
class BPA:
	...
	def run(self, feature_vectors, true_label):
		...
		loss = max(0, (1+((1-2*a)*total)))
		
		update_matrix = self.get_update_matrix(feature_vectors, predicted_label,total, loss, a)
		self.update_weights(update_matrix)
	...
	def get_update_matrix(self, feature_vectors, calculated_label, predicted_label, true_label, support_set,l_tilde,labeltil,c=0.1):
		...
		for i in range(0,len(update_matrix)):
			...
			if i != true_label:
				for j in range(0,len(feature_vectors)):
					update_matrix[i][j] = -feature_vectors[j] * step_lambda[i]
					# print("label is %s and update is %s" %(str(i), str(update_matrix[i][j])))
